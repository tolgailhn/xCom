...
def load_auto_reply_settings() -> dict:
    """Load auto self-reply settings"""
    path = DATA_DIR / "auto_reply_settings.json"
    if path.exists():
        with open(path, "r", encoding="utf-8") as f:
            saved = json.load(f)
            return {**_AUTO_REPLY_DEFAULTS, **saved}
    return _AUTO_REPLY_DEFAULTS.copy()


def save_auto_reply_settings(settings: dict):
...
def load_auto_reply_log() -> list[dict]:
    """Load auto self-reply log"""
    path = DATA_DIR / "auto_reply_log.json"
    if path.exists():
        with open(path, "r", encoding="utf-8") as f:
            return json.load(f)
    return []
...
def save_auto_reply_log(log: list[dict]):
    """Save auto self-reply log (max 500 entries)"""
    path = DATA_DIR / "auto_reply_log.json"
    os.makedirs(DATA_DIR, exist_ok=True)
    log = log[:500]
    with open(path, "w", encoding="utf-8") as f:
