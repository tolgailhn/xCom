...
const TABS = [
  ...
  { id: "autoreply", label: "Otomatik Reply" },
...
export default function AyarlarPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {activeTab === "autoreply" && <TabAutoReply />}
...
const STYLE_OPTIONS = [
  ...
  { value: "hook", label: "Hook / Dikkat Cekici" },
  { value: "teknik", label: "Teknik" },
  { value: "provoke", label: "Provoke Edici" },
  { value: "bilgilendirici", label: "Bilgilendirici" },
];

function TabAutoReply() {
  ...
  const [log, setLog] = useState<AutoReplyLogEntry[]>([]);
  const [loaded, setLoaded] = useState(false);

  useEffect(() => {
    Promise.all([getAutoReplySettings(), getAutoReplyLog()]).then(
      ([settings, logData]) => {
        const s = settings as {
          enabled: boolean;
        ...
        setInterval(s.check_interval_minutes);
        setLookback(s.lookback_hours);
        setStyle(s.reply_style);
        setLog(
          ((logData as { entries: AutoReplyLogEntry[] }).entries || [])
        );
        setLoaded(true);
      }
    );
  ...
  const handleRunNow = async () => {
    ...
    setRunResult(null);
    try {
      const result = await triggerAutoReply();
      setRunResult(result as typeof runResult);
      // Refresh log
      const logData = await getAutoReplyLog();
      setLog(
        ((logData as { entries: AutoReplyLogEntry[] }).entries || [])
      );
    } catch (e) {
      setRunResult({ success: false, errors: [String(e)] });
    } finally {
      setRunning(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card space-y-5">
        <h3 className="font-semibold text-white">Otomatik Self-Reply</h3>
        <p className="text-xs text-zinc-400">
          Tweet attiktan sonra AI otomatik olarak 1 adet self-reply atar.
          Bu, X algoritmasinda Phoenix ranking boost saglar.
          Her tweet&apos;e sadece 1 reply atilir, fazlasi atilmaz.
        </p>

        {/* Toggle */}
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm text-white">Otomatik Self-Reply</div>
            <div className="text-xs text-zinc-500">
              {enabled ? "Aktif — arka planda calisiyor" : "Kapali"}
            </div>
          </div>
          <button
        ...
        {/* Self-reply count info */}
        <div className="bg-zinc-900/50 rounded-lg p-3 text-xs text-zinc-400">
          Her tweet&apos;e sabit <span className="text-white font-semibold">1 adet</span> self-reply atilir.
          Zaten reply atilmis tweet&apos;ler otomatik atlanir.
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={running}
            className="text-sm px-6 py-2 border border-zinc-600 rounded-lg hover:border-blue-500/50 hover:text-blue-300 transition-colors disabled:opacity-40"
          >
        ...
        {runResult && (
          ...
          >
            {runResult.success !== false ? (
              <div>
                Kontrol edildi: {runResult.checked || 0} tweet |
                Reply atildi: {runResult.replied || 0} |
                Atlandi: {runResult.skipped || 0}
                {runResult.errors && runResult.errors.length > 0 && (
                  <div className="mt-1 text-xs text-yellow-300">
                    Hatalar: {runResult.errors.join(", ")}
                  </div>
                )}
              </div>
            ) : (
              <div>Hata: {runResult.errors?.join(", ") || "Bilinmeyen hata"}</div>
            )}
          </div>
        )}
      </div>
      ...
      <div className="glass-card space-y-4">
        ...
        {log.length > 0 ? (
            ...
            {log.map((entry, i) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  >
                    {entry.status === "success" ? "Basarili" : "Basarisiz"}
                  </span>
                  <span className="text-xs text-zinc-500">
                    {entry.replied_at?.slice(0, 16)}
                  </span>
                </div>
                <div className="text-xs text-zinc-500">
                  Orijinal tweet:
                ...
                <p className="text-sm text-zinc-400">
                  {entry.tweet_text?.slice(0, 150)}
                  {(entry.tweet_text?.length || 0) > 150 ? "..." : ""}
                </p>
                <div className="text-xs text-zinc-500">Reply:</div>
                <p className="text-sm text-zinc-300">{entry.reply_text}</p>
                {entry.reply_url && (
                  <a
                ...
                {entry.error && (
                  <p className="text-xs text-red-400">Hata: {entry.error}</p>
                )}
              </div>
            ))}
          </div>
          ...
          <p className="text-sm text-zinc-500">
            Henuz otomatik reply atilmamis.
          </p>
        )}
      </div>
    </div>
